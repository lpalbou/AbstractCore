...
- **[Memory and Model Residency](memory-management.md)** — host memory snapshot (`get_memory_snapshot`), host-wide resident-model sweep (`sweep_loaded_models`), per-provider loaded-model listings, what `unload_model()` frees (weights + session caches) and how to verify it, ejecting an MLX model from every holder (`eject_model`), gateway model locks (`/acore/models/lock`), and context calibration/estimation (`estimate_context_fit`)
