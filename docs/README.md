...
- **[Memory and Model Residency](memory-management.md)** — host memory snapshot (`get_memory_snapshot`), host-wide resident-model sweep (`sweep_loaded_models`), per-provider loaded-model listings, what `unload_model()` frees (weights + session caches) and how to verify it, ejecting a model from the whole process across MLX, HuggingFace (transformers and GGUF) and embedding models (`process_residency.eject` / `eject_unclaimed` with the claims registry), the one-figure `device.process_held_bytes`, gateway model locks (`/acore/models/lock`), and context calibration/estimation (`estimate_context_fit`)
