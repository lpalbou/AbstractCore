...
- `reasoning_levels` - Ordered list of request-side effort levels the model's API accepts (subset of `["none", "minimal", "low", "medium", "high", "xhigh"]`). `"none"` means the API accepts an explicit no-reasoning effort value (e.g. `reasoning_effort: "none"` on gpt-5.1+); it is a disable signal, not an effort level. Absent list means the model advertises no effort enum (boolean/template controls may still exist via `thinking_control`).
- `thinking_budget` - Model supports a thinking token budget knob (boolean; e.g. Claude extended thinking `budget_tokens`, Seed-OSS `thinking_budget` template kwarg).
- `thinking_control_mode` - Anthropic thinking transport for this model: `"adaptive"` (`thinking: {"type": "adaptive"}` + `output_config.effort`) or `"budget"` (`thinking: {"type": "enabled", "budget_tokens": N}`; the only mode on Claude 4.5 and earlier).
- `thinking_disable_supported` - Whether the API accepts an explicit disable artifact (boolean; default true when absent). Set `false` for adaptive-only Claude models (Opus 4.7, Fable 5, Mythos 5) that reject `thinking: {"type": "disabled"}` with HTTP 400 — providers then express thinking-off by omitting the `thinking` parameter, with a warning.
- `max_effort_supported` - Whether the API accepts an effort value above `xhigh` (Anthropic `effort: "max"`; boolean). Currently informational: the unified `thinking=` vocabulary tops out at `xhigh` and providers never escalate a request silently.
